...
export default function CommentUpdate({
  ...
  // لما المودال يفتح، احط البيانات القديمة
  useEffect(() => {
    if (isOpen && comment) {
      if (textRef.current) textRef.current.value = comment.content || "";
      setPreviewImage(comment.image || null);
    }
  }, [isOpen, comment]);
  ...
  function getForm() {
    ...
    formData.append("content", textRef.current.value.trim() || "");
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async () => {
      ...
      await query.refetchQueries({ queryKey });
      onOpenChange(false);
    },
              ...
              {previewImage && (
                <div className="relative rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                  <img
                    src={previewImage}
                    alt="معاينة التعليق"
                    className="max-h-[280px] w-full object-contain bg-black/5"
                  />
                  <button
                    onClick={removeImage}
